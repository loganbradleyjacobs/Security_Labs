...
def cross_correlation(dict1: dict[str, float], dict2: dict[str, float], symbols: Symbol_Set = None) -> list[float]:
    '''
...
def get_caesar_shift(enc_message: str, expected_dist:dict[str, float], symbols: Symbol_Set = None) -> int:
    '''
    Gets the likely shift used to originally encrypt a caesar cipher.<br>
    ...
    '''
    symbols = Utils._default_set(symbols)
    cc = cross_correlation(frequency_analysis(enc_message, symbols), expected_dist, symbols)
    return cc.index(max(cc))

def get_vigenere_keyword(enc_message: str, size: int, expected_dist: dict[str, float], symbols: Symbol_Set = None) -> str:
    '''Gets the likely keyword used to originally encrypt a vigenere cipher.'''
    keyword = ""
    if size == 0:
        return keyword
    messages = columnize(enc_message, size)
...
def columnize(enc_message: str, size: int) -> list[str]:
    """splits the encrypted message into size columns."""
...
def main():
    common_symbol_sets = {
        ...
        'alphabet_with_space': Symbol_Set(" ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz")
    }
    ...
    print("(2.2)--------------------------------")
    print(message)
    print(f"Frequency Analysis: {frequency_analysis(message, common_symbol_sets['ASCII_printables'])}")
    ...
    print(f"set2: {set2}")
    print(f"set3: {set3}")
    print(f"Cross-Correlation between Set 1 & 2: {cross_correlation(set1, set2, common_symbol_sets['A_through_F'])}")
    print(f"Cross-Correlation between Set 1 & 3: {cross_correlation(set1, set3, common_symbol_sets['A_through_F'])}")
    
    # 2.4
    print("(2.4)--------------------------------")
    C = caesar_cipher(message.upper(), 7, True, common_symbol_sets['upper_alphabet_with_space'])
    print(f"Message: {message.upper()}")
    print(f"Ciphertext (using Caesar Cipher): {C}")
    english_dist = {' ': .1828846265,'E': .1026665037, 'T': .0751699827, 'A': .0653216702, 'O': .0615957725, 'N': .0571201113, 'I': .0566844326,'S': .0531700534,'R': .0498790855,'H': .0497856396,'L': .0331754796,'D': .0328292310,'U': .0227579536,'C': .0223367596,'M': .0202656783,'F': .0198306716,'W': .0170389377,'G': .0162490441,'P': .0150432428,'Y': .0142766662,'B': .0125888074,'V': 0.0079611644,'K': 0.0056096272,'X': 0.0014092016,'J': 0.0009752181,'Q': 0.0008367550,'Z': 0.0005128469}
    likely_shift = get_caesar_shift(C, english_dist, common_symbol_sets['upper_alphabet_with_space'])
    print(f"Likely shift used in Caesar Cipher Encryption: {likely_shift}")
    decrypted_message = caesar_cipher(C, likely_shift, False, common_symbol_sets['upper_alphabet_with_space'])
    ...
    for mj in [m1, m2, m3]:
        ...
        for i in range(0,9):
            ...
            print(f"keylength = {i}, tried keyword: '{likely_keyword}' -------------------------------------------")
